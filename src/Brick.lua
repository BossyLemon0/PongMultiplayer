...
function Brick:init(x, y)
    ...
    --used to determine if brick has power
    self.hasPower = false

    --which power to trigger in powers table
    self.powNum = 0
...
function Brick:hit(power, haskey)
    ...
    if self.isKey == false then
        if self.tier > 0 and power ~= 8 then
        ...
        elseif self.tier > 0 and power == 8 then
            if self.color == 1 then
                self.tier = self.tier - 1
                self.color = 4
    ...
    elseif self.isKey == true and haskey > 0 then
        self.inPlay = false
    end
...
function Brick:render()
    if self.inPlay and self.isKey == false then
